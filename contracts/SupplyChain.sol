...
contract SupplyChain {
  ...
  */
   ...
   */
    // given sku item should be for sale
    modifier forSale(uint256 _sku) {
        // check if price is not 0 to indicate item for sale
        // will not handle negative, or 0 price, no bound restrictions use caution
        require(items[_sku].price != 0);
        _;
    }

    // verify item is marked as sold given sku
    modifier sold(uint256 _sku) {
        require(items[_sku].state == State.Sold);
        _;
    }
    // verify item is marked as shipped given sku
    modifier shipped(uint256 _sku) {
        require(items[_sku].state == State.Shipped);
        _;
    }
    // verify item is marked as received given sku
        ...
        paidEnough(items[sku].price)
    {
        ...
        // set the buyer as caller
        items[sku].buyer = msg.sender;
        // set state to be sold
  ...
  is the buyer. Change the state of the item to received. Remember to call the event associated with this function!*/
    ...
    {
        // set state of item to received
        items[sku].state = State.Received;
        // log received item event
